...
const (
	...
	singleFail = 3			//after how much time will the master fail in TestSingleMasterFailure
	singleFailRecovery = 5		//after how much time will master recover in TestSingleMasterFailure

	multipleFailsInterval = 5	//on what interval will the master fail/recover in TestManyMasterFailures
)
...
/*test whether code can handle master failing once after a specified amount of time*/
func TestSingleMasterFailure(t *testing.T) {
	...
	// Start 2 workers
	w := port("worker0")
	go RunWorker(mr.address, w, MapFunc, ReduceFunc, -1)
	...
	for !done {
		...
		default:
			// Start 2 workers every two secs. 
			...
			if secs > singleFail && !hasFailed{
				go func() {
					fmt.Printf("\nTest: Simulating master failure!\n\n")
					mr.abortChannel <- true
					hasFailed = true
				}()

			}else if secs > singleFail + singleFailRecovery && !hasRebooted {	
...
/*test whether code can handle master failing once after a specified amount of time*/
func TestManyMasterFailures(t *testing.T) {
	mr := setup()
	...
	for !done {
		...
		default:
			time.Sleep(multipleFailsInterval * time.Second)
